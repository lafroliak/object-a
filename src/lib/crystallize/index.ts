function createApiCaller(uri: string) {
  return async function callApi({ query, variables, operationName }: unknown) {
    const response = await fetch(uri, {
      method: 'POST',
      headers: {
        'content-type': 'application/json',
        'X-Crystallize-Access-Token-Secret': process.env
          .CRYSTALLIZE_SECRET_TOKEN!,
        'X-Crystallize-Access-Token-Id': process.env
